...
(dir exists, ZFS has ~480 GB). **Applied 2026-09-10 on all 8 reachable hosts (.70
.71 .72 .228 .229 .230 .231 .232); runtime-only, resets on reboot** — re-apply
after any reboot and add a sysctl.d drop-in if it should persist. Same hazard for
the profiler itself: run it from a ZFS cwd. Also: /var/log/ceph logs were rotated
2026-09-04 and never reopened (0-byte files) -> use journalctl -u 'ceph-*' for
daemon output on these hosts.
